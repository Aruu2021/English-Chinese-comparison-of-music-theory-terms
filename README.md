# English-Chinese-comparison-of-music-theory-terms
音乐术语英汉对照，包括作曲理论、音乐史、民族音乐、流行音乐、音乐科技、音乐教育等领域术语（凡例及参考文献见文末）
## A 1
- accented 强位
- accidental 变音记号，临时变音记号（相对与调号而言）
- Aeolian mode 爱奥利亚调式
- alla breve 二二拍
- altered chord 变和弦，变化和弦
- alto 女低音；女低音（声）部，中音（声）部
- alto clef 女低音谱号，中音谱号
- anacrusis 弱起
- anticipation（ANT） 先现音
- appoggiatura 倚音
- approach 到达，（不协和音的）预备
- arpeggiation 琶音式，分解和弦式
- arpeggio 琶音
- ascending 上行
- ascending melodic minor scale 上行旋律小调音阶
- augmentation dot 附点（一般简称为dot）
- augmented 增（指音程或和弦性质）
- augmented dominant chord 增属和弦，升五音的属
- augmented second 增二度
- augmented sixth 增六度
- augmented sixth chord 增六和弦
- authentic 正格的
- authentic cadence（AC） 正格终止
## B 2
- balanced binary 方整型单二部曲式
- bar 小节
- bar line 小节线
- basic idea 基本乐思
- bass 男低音；男低音（声）部，低音（声）部；低音（和弦最低音）
- bass clef 低音谱号
- bass line 低音线
- bass position 低音位置，低音位
- bass-position symbol 低音位置标记，功能和弦标记
- bass staff 低音谱表
- beam 符杠
- beat 拍，拍子
- beat division 拍子细分（或称division of beat）（英美理论中，一拍细分成两份为单拍子，细分成三份为复拍子）
- binary form 单二部曲式，二段式，二部曲式
- borrowed chord 借用和弦，变音和弦
- bottom note （音程）下方音
- bottom voice 最低声部
- breve note 二全音符
- broken 分解
- broken chord 分解和弦
## C 3
- cadence 终止
- cadential six-four chord 终止四六和弦，K46和弦
- cantus firmus（C.F.） 固定调，固定旋律，固定声部
- caret 音级数字上面的^符号
- C clef c谱号，中音谱号
- choir n.合唱团，唱诗班 v.合唱
- chorale 众赞歌
- chorale format① 合唱风格（同chorale style）
- chorale style② 合唱风格
- chord 和弦
- chordal 和弦的
- chordal dissonance 和弦不协和音
- chordal seventh 和弦七音
- chord construction 和弦结构
- chord progression 和弦进行
- chord skip 和弦转换
- chord symbol 和弦标记
- chord tone 和弦音
- chromatic 半音的，色彩化的，变化的
- chromatic chord 半音和弦，变音和弦
- chromaticism 半音体系
- chromatic semitone 变化半音，色彩半音
- circle of fifth 五度循环圈
- clef 谱号
- closely related key 近关系调
- close position 同close structure
- close structure 密集排列
- closing section 结束部
- coda 尾声
- common chord 共同和弦
- common time 四四拍
- common tone 共同音；共同音保持
- composition 作曲
- compound beat③ 复拍子（同compound meter）
- compound equivalent 复音程等音
- compound meter① 复拍子（同compound beat）
- compound interval 复音程
- compound single 复一拍子
- compound ternary form 复三部曲式
- concerto 协奏曲
- conjunct motion 级进
- consonant （音程或和弦）协和
- continuous binary form 延续单二部曲式（第一部分不以主和弦结束）
- contrary motion 反行，反向进行
- contrasting period 对比乐段
- counterpoint 对位法；对位，对位声部，对位旋律
- contrapuntal motion 对位化运动
## D 4
- deceptive cadence（DC）  阻碍终止 
- deceptive progression 阻碍进行
- degree 音级
- degree number 音级数字
- degree symbol 减和弦的°标记
- descending 下行
- descending melodic minor scale 下行旋律小调音阶
- development 展开部
- diatonic chord 自然和弦
- diatonic scale 自然音阶
- diminished 减（指音程或和弦性质）
- diminished seventh chord 减七和弦（同fully diminished）
- direct fifth 隐伏五度
- direct octave 隐伏八度
- dissonance 不协和音（或音程、和弦）
- dissonant （音、音程或和弦）不协和的
- distantly related key 远关系调
- division of beat 见beat division
- dominant 属音；属七和弦的简称；属功能
- dominant seventh chord 属七和弦（简称dominant）
- dominant with substituted sixth 六度音的属
- Dorian mode 多利亚调式
- dot 附点（此为简称，全称是augmentation dot）
- dotted note 附点音符
- dotted-half note 附点二分音符
- dotted-quarter note 附点四分音符
- dotted-eighth note 附点八分音符
- dotted-sixteenth note 附点十六分音符
- double v.对某个声部进行八度重复
- double circle of fifth 双五度循环圈
- double dot 复附点
- double flat 重降号
- double mixture 二重混织*（详见mixture），指在primary mixture的基础上增加secondary mixture（详见这两个术语）
- double period 复乐段
- double sharp 重升号
- doubly augmented 倍增（指音程性质）
- doubly diminished 倍减（指音程性质）
- downbeat 强拍，下拍
- duple 二拍
- duple compound meter 复二拍子
- duple meter 二拍子
- duple simple meter 单二拍子
- duplet 二连音
- duration 时值
- durational 时值的
- durational symbol 时值符号
- dynamic 力度
## E 5
- eighth note 八分音符
- eighth-note rest 八分休止符
- eleventh chord 十一和弦
- elision 闯入终止，重叠
- embellish v.装饰
- embellishing tone 装饰音
- embellishment 装饰音
- energy 力性，力度性
- enharmonic 异名同音的，等音的
- enharmonic equivalent 等音（等音是一个专有名词，表示音高相同而记法和意义不同的音，此为意译，直译为“异名同音的等同音”，故同名同音的等同音以及八度等同音不能加enharmonic）
- enharmonic interval 等音程
- enharmonic modulation 等和弦转调
- equal-tempered 十二平均律的，十二均分的
- equivalence n.等同
- equivalent 等同音（比enharmonic equivalent包含的范围要大，也包括同名同音的等同音和八度等同音）
- escape tone 逸音，规避音，跳进解决的辅助音
- evaded cadence 规避终止⑤
- exposition 呈示部
- extensions 延伸音（另见tension）
## F 6
- false recapitulation 假再现
- F clef F谱号，低音谱号
- fifth 五度；（和弦的）五音
- fifth line （五线谱的）五线
- fifth space （五线谱的）五间
- fifth-species counterpoint 第五类对位
- figuration 音型
- figure （数字低音的）数字；音型
- figured bass 数字低音
- filled-in notehead 实心符头
- first inversion 第一转位
- first line （五线谱的）一线
- first space （五线谱的）一间
- first-species counterpoint 第一类对位
- five-part rondo form 五部回旋曲式
- flag 符尾
- flat 降号
- foreign relationship 远关系调
- form 曲式
- four-part harmony 四部和声
- fourth 四度
- fourth line （五线谱的）四线
- fourth space （五线谱的）四间
- fourth-species counterpoint 第四类对位
- French augmented sixth chord 法兰西增六和弦
- full score 总谱
- fully diminished seventh chord 减减七和弦（简称fully diminished）
- function 功能
- fundamental tone 基音
## G 7
- G clef G谱号，高音谱号
- German augmented sixth chord 德意志增六和弦
- grand staff 大谱表
## H 8
- half cadence（HC） 半终止
- half-diminished seventh chord 半减七和弦（简称half-diminished）
- half note 二分音符
- half-note rest 二分休止符
- half step 半音，半音级
- harmonic 和声的
- harmonic interval 和声音程
- harmonic minor scale 和声小调音阶
- harmonic motion 和声化运动
- harmonic overtone 泛音列
- harmonic procedure 同harmonic progression
- harmonic progression 和声进行，和声序进
- harmonics 泛音
- harmonic series 泛音列
- harmonize v.为...配和声
- harmonizing n. 配和声
- harmony 和声
- hemiola 赫米奥拉比例 （三比二）
- hidden fifth 隐伏五度
- hidden octave 隐伏八度
- home key 主调
- hymn 赞美诗
## I 9
- idea 见basic idea或musical idea
- imperfect 不完全，不完满（表示音程协和程度或终止完满程度）
- imperfect authentic cadence（IAC）不完全终止，不完满正格终止
- imperfect consonance 不完全协和（音程）
- incomplete neighbor tone（INT）不完全辅助音（装饰音的一种，级进到达跳进解决或跳进到达级进解决）
- inner voice 内声部
- instrument 乐器
- instrument music 器乐
- interval 音程
- interval inversion 音程转位
- interval quality 音程性质
- interval size 音程度数
- inversion n.（音程或和弦）转位
- invert v.（音程或和弦）转位
- Ionian mode 伊奥尼亚调式
- Italian augmented sixth chord 意大利增六和弦
## J 10
## K 11
- key 调，调性；（键盘乐器的）键
- keyboard （键盘乐器的）键盘
- keyboard format① 键盘风格（同keyboard style）
- keyboard style② 键盘风格
- key signature 调号
## L 12
- leading-tone 导音
- leading-tone seventh chord 导七和弦
- leap 跳进
- ledger line 加线
- line （五线谱的）线（具体某线在其前加序数词表示，如二线second line）
- Locrian mode 洛克里亚调式
- lower note 根音，下方音（一般指音程）
- lower voice 下方声部（泛指，具体哪些声部要在后面用括号注释）
- Lydian mode 利底亚调式
## M 13
- major 大（指调式、音程或和弦的性质）；做大调、大七和弦的简称
- major key 大调
- major-minor seventh chord 大小七和弦
- major scale 大调音阶
- major seventh chord 大七和弦，大大七和弦（简称major）
- material 材料
- measure 小节
- mediant 中音（音阶的三级音)
- melodic interval 旋律音程
- melodic line 旋律线
- melodic minor scale 旋律小调音阶
- melody 旋律
- meter 节拍
- metric accent 节律重音
- metronome 节拍器
- middle C 中央C
- minor 小（指调式、音程或和弦性质）；做小调、小七和弦的简称
- minor key 小调
- minor scale 小调音阶
- minor seventh chord 小七和弦（简称minor）
- minuet 小步舞曲
- Mixolydian mode 混合利底亚调式
- mixture 调式交替（见mode mixture），混织*（详见[文章1](https://zhuanlan.zhihu.com/p/20495090)、[文章2](https://zhuanlan.zhihu.com/p/544472697)）
- mode 调式
- mode mixture 调式交替，转旋
- modifier 音程性质
- modulate v.转调
- modulating sequence 转调模进
- modulation n.转调
- monophonic 单声部的
- motion 进行，运动（progression、voice leading和motion都有进行之意，progression指和声序进，voice leading指和声序进时声部的连接，而motion则是指声部连接的方式，如平行、反行、半音进行等）
- motive 动机
- motivic cell 动机细胞
- movement 乐章
- musical duration 音值
- musical idea 乐思
- musical score 乐谱
- musical tone 乐音
## N 14
- natural 还原号
- natural interval 自然音程
- natural minor scale 自然小调音阶
- natural triad 自然三和弦
- Neapolitan chord 那不勒斯和弦，拿波里和弦
- neighbor tone（NT） 辅助音，邻音
- nesting of sentence 嵌套句式
- ninth chord 九和弦
- non-chord tone 和弦外音
- notate v.记谱
- notation 记谱法
- note 音符
- notehead 符头（实心符头称filled-in notehead，空心符头称open notehead）
- numerical name 音程度数
## O 15
- oblique motion 斜行，斜向进行
- octave 八度
- octave doubling 八度重复音
- octave register 八度音区
- open notehead 空心符头
- open position 同open structure
- open structure 开放排列
- outer voice 内声部
- overtone 泛音
## P 16
- parallel fifth 平行五度
- parallel key 同主音调，平行调
- parallel minor 同主音小调
- parallel motion 平行进行
- parallel octave 平行八度
- parallel period 平行乐段
- parallel unison 平行一度
- part 声部
- part writing 声部写作
- passage 一段；一章
- passing chord 经过和弦
- passing six-four chord 经过四六和弦
- passing tone（PT） 经过音
- pedal point 持续音
- pedal six-four chord 辅助四六和弦
- perfect 纯 （指音程性质）；完全，完满（表示音程协和程度或终止完满程度）
- perfect authentic cadence（PAC） 完全终止，完满正格终止
- perfect consonance 完全协和（音程）
- period 乐段
- phrase 乐句
- phrase group 乐句组
- Phrygian half cadence（PHC） 弗里几亚终止
- Phrygian mode 弗里几亚调式
- picardy third 皮卡迪三度
- pickup 弱起（但一般用anacrusis表示弱起）
- piece 一部（作品）
- pitch 音高
- pitch class 音级，音高级
- plagal cadence（PC） 变格终止
- plagal progression 变格进行
- position 指和弦根音的各种位置
- pre-dominant 属前和弦，预属和弦
- preparation 预备音
- primary mixture 初阶混织*（详见mixture），指同主音调各音级上的调
- primary theme 主部主题
- principal pitch 主音
- progress v.进行
- progression n.进行，序进（与motion的区别见motion一词）
- pure tone 纯音
## Q 17
- quadruple 四拍
- quadruple compound meter 复四拍子
- quadruple meter 四拍子
- quadruple simple meter 单四拍子
- quadruplet 四连音
- quality 音色，音质；指音程或和弦性质
- quarter note 四分音符
- quarter-note rest 四分休止符
- quintuplet 五连音
## R 18
- range 音域
- realize v.对（记谱不全的乐曲）做完整改编，根据（和声标记）写出（四部和声）
- real sequence 半音模进，严格模进
- recapitulation 再现部
- reduced score 缩谱
- register 音区
- relative key 关系调（关系调也叫平行调，是因为俄文称关系调为平行调，影响了我国早期术语翻译，而同主音调英文使用了parallel这个词，与俄文相反）
- remain 保留，保持
- resolution n.解决；（不协和音的）解决音
- resolve v.解决
- rest 休止符
- retardation（RET） 一种上行解决的延留音
- retransition 属准备，回头过渡
- rhythm 节奏
- rhythmic 节奏的
- Roman numeral 罗马数字标记，罗马和弦标记
- rondo form 回旋曲式
- root 根音
- root position 原位
- rounded rinary form 再现单二部
## S 19
- scale 音阶
- scale degree 音级，音阶级
- scale-degree name 音级名
- scale-degree number 音级数字
- scale step 音级
- score 总谱
- second 二度
- secondary 副属
- secondary dominant chord 副属和弦
- secondary function 副属功能
- secondary key 副调
- secondary leading-tone chord 副导和弦
- secondary mixture 次阶混织*（详见mixture），指在大调自然音级上建立的小调的同主音大调，或在小调自然音级上建立的大调的同主音小调
- secondary theme 副部主题
- second inversion 第二转位
- second line （五线谱的）二线
- second space （五线谱的）二间
- second-species counterpoint 第二类对位
- sectional binary form 分节单二部曲式（第一部分以主和弦结束）
- sentence 句式，句子，[叠起段（或句）](https://zhuanlan.zhihu.com/p/461807565)，递增乐段
- semicadence 半终止
- semitone 半音
- septuplet 七连音
- sequence 模进
- seven-part rondo form 七部回旋曲式
- seventh 七度；（和弦的）七音
- seventh chord 七和弦
- sextuplet 六连音
- sharp 升号
- signature 见time signature或key signature
- similar fifth 隐伏五度
- similar motion 同向进行
- similar octave 隐伏八度
- simple beat③ 单拍子（同simple meter）
- simple meter① 单拍子（同simple beat）
- simple interval 单音程
- simple mixture 与primary mixture同义（见primary mixture）
- sixteenth note 十六分音符
- sixteenth-note rest 十六分休止符
- sixth 六度
- sixth chord 六和弦
- sixty-fourth note 六十四分音符
- sixty-fourth-note rest 六十四分音符
- skip （三度的）小跳；另见chord skip
- sonata form 奏鸣曲式
- sonata-rondo form 奏鸣回旋曲式
- soprano 女高音；女高音（声）部，高音（声）部
- space （五线谱的）间（具体某间在其前加序数词表示，如二间second space）
- spacing 排列法，四部和声的声部间距
- species counterpoint 分类对位法
- speed 速度
- STAB format 同chorale format
- staff 五线谱，谱表（staff的本意是谱表，一般做五线谱的简称，五线谱的完整名称是five-line staff）
- static motion 保持（声部进行的一种)
- stationary motion 保持（同static motion）
- stave 五线谱（staves是staff的复数形式，stave是原本不存在的词，是被误推出来的）
- stem n.符干；v.为...配置符干
- step n.音级，阶位*；v.级进
- stepwise/stepwire 级进
- strong beat 强拍
- subdominant 下属音；下属功能
- submediant 下中音
- sub-phrase 分句（乐句的一半，可能无终止）
- subtonic 下主音
- supertonic 上主音
- supertonic seventh chord 上主七和弦，II级七和弦
- support 伴奏
- suspension（SUS） 延留音
- syncopation 切分音
## T 20
- tempo 速度
- tendency tone 趋向音⑤，有倾向性的音⑥，限定进行音⑦，不稳定音
- tenor 男高音；男高音（声）部，次中音（声）部
- tenor clef 男高音谱号，次中音谱号
- tension chord 延伸和弦，张力和弦
- tension note 延伸音
- ternary form 单三部曲式，三段式
- tertian leap 三音跳进
- texture 织体
- theme 主题
- third 三度；（和弦的）三音
- third inversion 第三转位
- third line （五线谱的）三线
- third space （五线谱的）三间
- third-species counterpoint 第三类对位
- thirteenth chord 十三和弦
- thirty-second note 三十二分音符
- thirty-second-note rest 三十二分休止符
- thoroughbass 通奏低音
- three-phrase period 三句乐段
- tie 延音线
- time signature 拍号
- tonal 调性的
- tonal music 调性音乐
- tonal sequence 自然音模进，守调模进
- tone 音
- tonic 主音；主功能
- tonicization 离调（主音化）
- tonicized triad 临时主和弦（主音化三和弦）
- top note （音程）上方音
- top voice 最高声部
- transition 连接部
- transpose v.移调
- transposition n.移调
- treble clef 高音谱号
- treble staff 高音谱表
- triad chord 三和弦（或简称triad）
- trio 三声中部
- triple 三拍
- triple compound meter 复三拍子
- triple meter 三拍子
- triple simple meter 单三拍子
- triplet 三连音
- tritone 三全音
- tuplet 连音符
## U 21
- unaccented 弱位
- unbalanced binary 非方整型单二部曲式
- unequal fifth 不相等五度（指纯五度与减五度连接这类的进行）
- unison 同度，一度
- upbeat 弱拍，上拍；弱起（一般用anacrusis表示弱起）
- upper note 冠音，上方音（一般指音程）
- upper voice 上方声部（泛指，具体哪些声部要在后面用括号注释）
## V 22
- value 音值，时值
- violists 中提琴演奏者
- vocal choir 声乐合唱；声乐合唱团
- vocal music 声乐
- voice 声部
- voice crossing 声部交叉
- voice exchange 声部交换⑥，单音交换⑤
- voice leading 声部进行，声部导进，声部连接（与motion的区别见motion一词）
- voice overlap 声部超越
## W 23
- weak beat 弱拍
- whole note 全音符
- whole-note rest 全休止符
- whole step 全音，全音级
- whole tone 全音
## XYZ

凡例与备注
- 1.部分译名词性多，容易误解，会在译名前临时增加词性英文缩写表示词性，n.表示名词、v.表示动词、a.表示形容词、ad.表示副词。
- 2.多个不同译名的，按从常见到罕见排列（但这个基于编者主观判断，并不一定准确）。
- 3.分号隔开的译名为同词异意，逗号隔开的译名为同意异译
- 4.英文术语中，意思相同但词汇使用习惯不同的术语在其后标注英文参考文献的编号（外加圆圈）
- 5.中文译名中，有多个争议译名的术语在其后标注中文参考文献的编号（外加圆圈）
- 6.（施工中）

英文参考文献
- [1]L. Poundie Burstein & Joseph N. Straus, Concise Introduction to Tonal Harmony, W. W. Norton, 2016.
- [2]Steven G. Laitz , The Complete Musician, Oxford University Press, 2016.
- [3]Stefan Kostka & Dorothy Payne, Tonal Harmony, McGraw-Hill, 2017.

中文参考文献
- [1]李重光. 音乐基础理论[M]. 人民音乐出版社, 1962．
- [2]周复三. 音乐基础理论教程[M]. 山东大学出版社, 2005．
- [3]（苏）斯波索宾 等. 和声学教程[M]. 陈敏, 译. 人民音乐出版社, 2008．
- [4]（为Burstein与Straus所编的和声教材之中文版留坑）
- [5]（美）斯蒂芬·雷兹. 完全音乐理论教程[M]. 孟酋、孟巫娜, 译. 湖南文艺出版社, 2016．
- [6]（美）斯蒂凡·库斯特卡、多萝茜·佩恩. 调性和声及20世纪音乐概述[M]. 杜晓十, 译. 人民音乐出版社, 2010．
- [7]（日）島岡譲 等. 総合和声[M]. 音楽之友社, 1998（一些没有固定译法的术语提供日语汉字词做参考）
- [8]钱仁康、钱亦平. 音乐作品分析教程[M]．上海音乐出版社,2003．
- [9]于苏贤. 复调音乐教程[M]. 上海音乐出版社, 2001．

参考文献（词典、翻译类）
- 马扬芝. 中英双语音乐理论概述[M]．上海音乐学院出版社,2019．
- 姚恒璐. 实用音乐英文选读[M]. 湖南文艺出版社, 2010．
- 王田、洛秦. 英汉汉英音乐词汇手册[M]. 上海外语教育出版社, 2013．
- 罗伯托·勃拉奇尼. 音乐术语对照词典[M]．朱建、饶文心, 译. 上海音乐学院出版社,2003．
- 郑玉章、张愉、缑斌. 音乐分类词汇手册[M]. 天地出版社, 2005．

后记
本文现收词504个，编者学力所限，难免有所疏漏，欢迎大家在评论区留言讨论

